...
contract UniswapV2Pair is IUniswapV2Pair, ERC20 {
    ...
    // Based on: https://github.com/Vectorized/soledge/blob/main/src/utils/ReentrancyGuard.sol
    ...
    function addLiquidity(
        ...
        uint256 amount1Min,
        address to
    ) external override lock returns (uint256 amount0, uint256 amount1, uint256 liquidity) {
        if (reserve0 == 0 && reserve1 == 0) {
            (amount0, amount1) = (amount0Desired, amount1Desired);
        } else {
            uint256 amount1Optimal = quote(amount0Desired, reserve0, reserve1);
            if (amount1Optimal <= amount1Desired) {
                require(amount1Desired >= amount1Min, "UniswapV2Pair: INSUFFICIENT_B_AMOUNT");
                (amount0, amount1) = (amount0Desired, amount1Optimal);
            } else {
                uint256 amount0Optimal = quote(amount1Desired, reserve1, reserve0);
                assert(amount0Optimal <= amount0Desired);
                require(amount0Optimal >= amount0Min, "UniswapV2Pair: INSUFFICIENT_A_AMOUNT");
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        require(amount0 >= amount0Min, "UniswapV2Pair: INSUFFICIENT_0_AMOUNT");
        require(amount1 >= amount1Min, "UniswapV2Pair: INSUFFICIENT_1_AMOUNT");
    }
    ...
    function swapExact(uint256 amountIn, uint256 amountOutMin, bool inIsToken0, address to) external lock {
        uint256 amountOut = getAmountOut(amountIn, inIsToken0);

        require(amountOut >= amountOutMin, "UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT");
        SafeTransferLib.safeTransferFrom(inIsToken0 ? token0 : token1, msg.sender, address(this), amountIn);

        _swap(inIsToken0 ? 0 : amountOut, inIsToken0 ? amountOut : 0, to);
    }
    ...
    // HELPERS
    function getAmountOut(uint256 amountIn, bool inIsToken0) internal view returns (uint256 amountOut) {
        require(amountIn > 0, "UniswapV2Pair: INSUFFICIENT_INPUT_AMOUNT");
        uint256 reserveIn = inIsToken0 ? reserve0 : reserve1;
        uint256 reserveOut = inIsToken0 ? reserve1 : reserve0;

        require(reserveIn > 0 && reserveOut > 0, "UniswapV2Pair: INSUFFICIENT_LIQUIDITY");
    ...
    function quote(uint256 amountA, uint256 reserveA, uint256 reserveB) internal pure returns (uint256 amountB) {
        require(amountA > 0, "UniswapV2Pair: INSUFFICIENT_AMOUNT");
        require(reserveA > 0 && reserveB > 0, "UniswapV2Pair: INSUFFICIENT_LIQUIDITY");
